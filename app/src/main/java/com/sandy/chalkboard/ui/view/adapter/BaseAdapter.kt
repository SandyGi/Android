...
abstract class BaseAdapter<T, V>(private val mContext: Context, private var mArrayList: ArrayList<T>?) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        onBindData(mArrayList!![position], position, (holder as BaseAdapter<T, V>.ItemViewHolder).mDataBinding)

        (holder.mDataBinding as View).setOnClickListener { onItemClick(mArrayList!![position], position) }
    }
